...
## Riferimenti generici (tipizzati a Variant) ai sottosistemi del gioco attivo:
## le classi concrete sono scelte dal Manifest del game_id (vedi GameRegistry).
var module: RulesModule
var game_def: GameDef
var state: GameState
var ops          ## sistema Operazioni (es. CubaLibreOperations)
var specials     ## sistema Attività Speciali
var propaganda   ## sistema Propaganda / round periodico
var events       ## sistema Eventi
var bot          ## sistema Non-giocatore (es. CLCalixto per Cuba Libre)
...
func new_game(scenario: String = "standard") -> void:
	...
	ops = GameRegistry.create_operations(state, module)
	specials = GameRegistry.create_specials(state, module)
	propaganda = GameRegistry.create_propaganda(state, module)
	events = GameRegistry.create_events(state, module)
	bot = GameRegistry.create_bot(state, module)
	stats = {}
	build_deck()
	advance_card()
...
## Ruolo di ogni Fazione: "player" (umano) o "bot" (Non-giocatore).
## Popolato da `_init_default_roles()` (delega al Manifest del gioco attivo)
## la prima volta che si avvia una partita. La referenza Dictionary è condivisa
## con `state.roles` — la mutiamo sul posto invece di riassegnarla.
var roles: Dictionary = {}

func _init_default_roles() -> void:
	var defaults: Dictionary = GameRegistry.default_roles(game_def)
	roles.clear()
